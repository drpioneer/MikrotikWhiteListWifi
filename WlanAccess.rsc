...
# tested on ROS 6.49.17 (capsman) & ROS 7.16.1 (wifiwave2 or wifi-qcom)
# updated 2024/11/28

:do {
  :local AvailableWifi do={ # wifi package search function
    :local counter 0; :local package ""
    :do {
  ...
  :local ReplacingChars do={ # character replacement function
    ...
    :local source {"\22"}; :local destin {"\27"}; :local result ""
    :for i from=0 to=([:len $1]-1) do={
      :local char [:pick $1 $i]; :local index [:find $source $char]
      :if ($index>-1) do={:set $char [:pick $destin $index]}
      :set $result "$result$char"}
    :return $result}

  # main body of the script
  :local counter 0; :local interface [$AvailableWifi]
  [[:parse "$interface access-list remove [find action=reject]"]]
  ...
  :foreach wlanClients in=[/ip dhcp-server lease find comment~"wifi"] do={
    ...
    :foreach accessList in=[[:parse "$interface access-list find"]] do={
      ...
      :local accessComment ([[:parse "$interface access-list get $accessList comment"]])
      :if ($wlanMAC=$accessMAC) do={
        :set $presence true
        :if ($wlanComment!=$accessComment) do={
          [[:parse "$interface access-list remove [find mac-address=$wlanMAC]"]]; :set $presence false}}}
    :if (!$presence) do={[[:parse "$interface access-list add mac-address=$wlanMAC comment=\"$wlanComment\" action=accept"]]}}
  [[:parse "$interface access-list add action=reject"]]} on-error={ # when emergency break script ->
